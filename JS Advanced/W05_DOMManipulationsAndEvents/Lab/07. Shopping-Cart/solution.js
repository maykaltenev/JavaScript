function solve() {
   const arrOfAddButtons = Array.from(document.getElementsByClassNameByClassName('add-product'));
   arrOfAddButtons.forEach(ev=> { ev.addEventListener('click', onClick)});
   document.getElementsByName('checkout')[0].addEventListener('click', onCheckout);
   ...
   function onClick(el){
      let productName = el.target.parentNode.parentNode.getElementsByClassNameByClassName('product-title')[0].textContent;

      let priceOfElement = Number(el.target.parentNode.parentNode.getElementsByClassNameByClassName('product-line-price')[0].textContent);
